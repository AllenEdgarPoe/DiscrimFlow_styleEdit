...
def main(args):
    ...
    testset = ImgDatasets(root_dir='/home/jkshark/data/celeba/images', files='./data/test_files.txt', mode='gray', train=False)
    testloader = data.DataLoader(testset, batch_size=16, shuffle=False, num_workers=1)
    # original = cond_data["original"]
    original ,cond_img = next(iter(testloader))
    original, cond_img = original.to(device), cond_img.to(device)
    ...
    torchvision.utils.save_image(origin_concat, args.output_dir + 'original_16.png')
    torchvision.utils.save_image(img_concat, args.output_dir + '/synthesized_16.png')
...
def style_transfer2(net, original, cond_img, target_index=0, img_size=64):
    B = 1
    target = [cond_img[target_index] for _ in range(B)]
    target = torch.stack(target)
    z, _  = net(original, cond_img)
    new_z = (z[2] - z[3]).unsqueeze(0)
    reconstructed, _ = net(new_z, target, reverse=True)
    reconstructed = torch.sigmoid(reconstructed)
    return reconstructed, cond_img[target_index]
    # B = original.size(0)
    # target = [cond_img[target_index] for _ in range(B)]
...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--ckpt_path', default="ckpts/best_big.pth.tar")
